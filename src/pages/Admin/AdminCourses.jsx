...
function AdminCourses() {
	...
	const onFinish = async (values) => {
		try {
			...
			let response;
			if (selectedItemForEdit) {
				response = await axios.put(`${baseURL}/api/portfolio/update-course`, {
					...values,
					_id: selectedItemForEdit._id,
				});
			} else {
				response = await axios.post(
					`${baseURL}/api/portfolio/add-course`,
					values
				);
			}
	...
	const onDelete = async (item) => {
		try {
			...
			const response = await axios.post(
				`${baseURL}/api/portfolio/delete-course`,
				{
					_id: item._id,
				}
			);
			...
			<div className="grid grid-cols-3 gap-5 sm:grid-cols-1 lg:grid-cols-2">
				{certification.map((course) => (
					<div
						...
						<h1 className="text-primary text-2xl font-bold">{course.title}</h1>
						<hr />
						<img src={course.image} alt="project" className="pt-2 h-60 w-80" />
				...
				>
					...
					<a>
						<Form.Item name="link" label="Link">
							<input placeholder="Link of Certification" />
						</Form.Item>
					</a>
...
//"certification" is used while destructuring because in databse we have "certification" instead of "courses" so while retriving data we hace used certification  and "courses" is used while mapping.
